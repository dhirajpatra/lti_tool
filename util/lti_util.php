...
require_once $_SERVER['DOCUMENT_ROOT'].'/lti_v2/library/OAuth.php';
...
function is_lti_request()
{
    ...
    if ($good_message_type and $good_lti_version) {
        return true;
    }
...
class BLTI
{
    ...
    public function getResourceLKey()
    {
        $id = $this->info['resource_link_id'];
        if (strlen($id) > 0) {
            return $id;
    ...
    public function setResourceID($new_id)
    {
    ...
    public function setConsumerID($new_id)
    {
    ...
    public function setCourseID($new_id)
    {
    ...
    public function getOutcomeService()
    {
    ...
    public function getOutcomeSourceDID()
    {
...
function handleOAuthBodyPOST($oauth_consumer_key, $oauth_consumer_secret)
{
    ...
    $request_headers['Authorization'] = 'OAuth realm="",oauth_version="1.0",oauth_nonce="30ff93f3b38aeb4342ef45aa2bdbd0fe",oauth_timestamp="1534499654",oauth_consumer_key="98765asd",oauth_body_hash="b33Izwp%2BrObZNqP8NJ3BnbD%2BjVs%3D",oauth_signature_method="HMAC-SHA1",oauth_signature="0eaLeJFtAEPSUKp6gGGupOIsuQA%3D"';
    ...
    //COMMENTED BY SRINIVASU - SINCE TEMP JSON DATA IS BEEN PASSING AS AN CURL POST ARRAY DATA
...
function get_curl($url, $header)
{
    ...
    if ($error_msg = curl_error($ch)) {
        echo "Launch request curl failed. \n".$error_msg;
        die();
    }
...
function body_socket($endpoint, $method, $data, $moreheaders = false)
{
    ...
    $hostport = ':'.$url['port'];
    if ($url['scheme'] == 'http' && $hostport == ':80') {
        $hostport = '';
    }
    if ($url['scheme'] == 'https' && $hostport == ':443') {
        $hostport = '';
    }
